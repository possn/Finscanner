(() => {
  ...
  function stockPortfolioCardHtml(r, meta = {}) {
    ...
    return `<article class="stock-portfolio-card" data-stock-portfolio-open="${escapeHtml(r.ticker)}">
      <div class="stock-portfolio-card__top"><div><span class="eyebrow">${escapeHtml(r.ticker)}</span><h4>${escapeHtml(r.name||r.ticker)}</h4>${r.business_summary?`<small class="stock-discover-summary">${escapeHtml(r.business_summary)}</small>`:''}</div>${Number.isFinite(score)?`<strong>${Math.round(score)}<small>/100</small></strong>`:''}</div>
  ...
  function congressionalActivityShellHtml(r){
    return `<div class="congress-activity-panel"><div class="section-heading"><div><span class="eyebrow">CONGRESSIONAL ACTIVITY · STOCK ACT</span><h3>Congressistas: quem comprou ou vendeu?</h3><p>Não repetimos um segundo gráfico. As transações do Congresso são sobrepostas no gráfico Smart Money acima; aqui fica o detalhe por pessoa.</p></div><span class="congress-balance" data-congress-balance>—</span></div><div class="congress-summary" data-congress-summary><span>A carregar divulgações recentes…</span></div><div class="insider-chart-controls" data-congress-controls><button class="is-active" data-congress-filter="all">Todos</button><button data-congress-filter="buy">Compras</button><button data-congress-filter="sell">Vendas</button><button data-congress-filter="senate">Senado</button><button data-congress-filter="house">House</button></div><div class="congress-ledger" data-congress-ledger></div><p class="detail-note">Os montantes são intervalos declarados (PTR House Clerk e Senate eFD). Nota: a fonte gratuita que agregávamos automaticamente ficou indisponível (passou a serviço pago) — esta secção pode estar sem dados novos até termos uma fonte gratuita de substituição.</p></div>`;
  }
  ...
  function insiderActivitySectionHtml(r) {
    ...
    return `<section class="insider-activity-panel">
      <div class="section-heading"><div><span class="eyebrow">SMART MONEY · SEC FORM 4</span><h3>Compras e vendas de insiders</h3><p>Um único gráfico de preço: insiders SEC + transações divulgadas por membros do Congresso.</p></div><span class="insider-year-net ${(Number(buyV||0)-Number(sellV||0))>=0?'positive-text':'negative-text'}">${fmtMoney(Number(buyV||0)-Number(sellV||0),r.currency||'USD')}</span></div>
  ...
  function drawInsiderChart(r, filter='all') {
    ...
    if(history.length<2){ctx.fillStyle='#8a93a1';ctx.font='13px system-ui';ctx.fillText('Histórico de preço indisponível neste run. O pipeline agora tenta Yahoo em batch e faz fallback individual.',18,40);return;}
    ...
    const markers=[];
    txs.forEach(tx=>{if(!tx.date||parse(tx.date)<minT||parse(tx.date)>maxT)return;const base=Number(tx.price);const yprice=Number.isFinite(base)?base:nearestPrice(tx.date).close;const x=xOf(tx.date),y=yOf(yprice);const size=6+Math.min(8,Math.sqrt(Math.max(0,Number(tx.value)||0)/maxV)*8);ctx.fillStyle=tx.type==='buy'?'#17a673':'#e34e59';ctx.beginPath();if(tx.type==='buy'){ctx.moveTo(x,y-size);ctx.lineTo(x-size,y+size*.7);ctx.lineTo(x+size,y+size*.7)}else{ctx.moveTo(x,y+size);ctx.lineTo(x-size,y-size*.7);ctx.lineTo(x+size,y-size*.7)}ctx.closePath();ctx.fill();markers.push({x,y,size,tx,source:'insider'});});
    const ctrades=Array.isArray(r.__congressTrades)?r.__congressTrades:[];
    ctrades.forEach(t=>{const d=t.transaction_date;if(!d||parse(d)<minT||parse(d)>maxT)return;const p=nearestPrice(d);const x=xOf(d),y=yOf(p.close);const size=6;ctx.lineWidth=2;ctx.strokeStyle=t.type==='buy'?'#2463a8':'#9a5a24';ctx.fillStyle='#fffaf0';ctx.beginPath();if(t.type==='buy'){ctx.moveTo(x,y-size);ctx.lineTo(x-size,y+size*.7);ctx.lineTo(x+size,y+size*.7)}else{ctx.moveTo(x,y+size);ctx.lineTo(x-size,y-size*.7);ctx.lineTo(x+size,y-size*.7)}ctx.closePath();ctx.fill();ctx.stroke();markers.push({x,y,size:8,tx:t,source:'congress'});});
    canvas._insiderMarkers=markers;
    canvas.onclick=e=>{const rect=canvas.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top;let m=null,dist=Infinity;for(const z of markers){const d=Math.hypot(x-z.x,y-z.y);if(d<dist){dist=d;m=z}}if(!m||dist>28){if(tooltip)tooltip.hidden=true;return;}const tx=m.tx;if(tooltip){tooltip.hidden=false;if(m.source==='congress'){tooltip.innerHTML=`<strong>${tx.type==='buy'?'COMPRA':'VENDA'} · ${escapeHtml(tx.member||'Congresso')}</strong><span>${escapeHtml((tx.chamber||'').toUpperCase())} · ${escapeHtml(tx.amount_range||'—')}</span><span>transação ${escapeHtml(tx.transaction_date||'—')} · divulgação ${escapeHtml(tx.disclosure_date||'—')}</span>`;}else{tooltip.innerHTML=`<strong>${tx.type==='buy'?'COMPRA':'VENDA'} · ${escapeHtml(tx.owner||'Insider')}</strong><span>${escapeHtml(tx.role||'')}</span><span>${escapeHtml(tx.date||'—')} · ${tx.shares==null?'—':Number(tx.shares).toLocaleString('pt-PT')} ações · ${tx.price==null?'—':Number(tx.price).toFixed(2)} · ${fmtMoney(tx.value,r.currency||'USD')}</span>`;}tooltip.style.left=Math.max(6,Math.min(cssW-260,m.x-100))+'px';tooltip.style.top=Math.max(8,m.y-86)+'px';}};
  }
  ...
  function fundPortfolioFit(r) {
    ...
    const observedDims = (Number.isFinite(fee)?1:0) + (Number.isFinite(aum)&&aum>0?1:0) + (holdings.length?2:0) + (maxOverlap!=null?1:0);
    const overall = observedDims >= 2 ? Math.round(costScore*.22 + sizeScore*.18 + diversificationScore*.28 + concentrationScore*.17 + (role === 'Core' ? 92 : role === 'Diversifier' ? 82 : role === 'Income' ? 72 : role === 'Redundant' ? 35 : 65)*.15) : null;
    return { costScore:Number.isFinite(fee)?costScore:null, sizeScore:(Number.isFinite(aum)&&aum>0)?sizeScore:null, diversificationScore:holdings.length?diversificationScore:null, concentrationScore:holdings.length?concentrationScore:null, overall, observedDims, role, roleReason, maxOverlap, overlaps, observedCoverage, top5 };
  }
  ...
  function drawFundPriceChart(r) {
    ...
    const pad={l:10,r:10,t:14,b:24}, vals=pts.map(x=>x.close), lo=Math.min(...vals), hi=Math.max(...vals), span=Math.max(hi-lo,Math.max(1,hi)*.04);
    const X=i=>pad.l+(w-pad.l-pad.r)*(i/(pts.length-1)); const Y=v=>pad.t+(h-pad.t-pad.b)*(1-(v-lo)/span);
    const css=getComputedStyle(document.documentElement), line=css.getPropertyValue('--accent').trim()||'#b57821', grid=css.getPropertyValue('--line').trim()||'rgba(80,70,50,.15)', text=css.getPropertyValue('--muted').trim()||'#6f7469';
    ctx.strokeStyle=grid; ctx.lineWidth=1; for(let i=0;i<3;i++){const y=pad.t+(h-pad.t-pad.b)*i/2;ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(w-pad.r,y);ctx.stroke();}
    ctx.beginPath(); pts.forEach((p,i)=>{const x=X(i),y=Y(p.close); i?ctx.lineTo(x,y):ctx.moveTo(x,y)}); ctx.strokeStyle=line;ctx.lineWidth=2.5;ctx.stroke();
    ctx.lineTo(X(pts.length-1),h-pad.b);ctx.lineTo(X(0),h-pad.b);ctx.closePath();ctx.globalAlpha=.10;ctx.fillStyle=line;ctx.fill();ctx.globalAlpha=1;
    ctx.font='11px system-ui';ctx.fillStyle=text;ctx.fillText(String(pts[0].date||'').slice(0,7),pad.l,h-6);const end=String(pts.at(-1).date||'').slice(0,7);ctx.fillText(end,w-pad.r-ctx.measureText(end).width,h-6);
  }
  ...
  function openFundDetail(r) {
    ...
    els.detailContent.innerHTML = `
      ...
      ${fundPriceChartHtml(r)}

      <section class="fund-role-hero">
        <div class="fund-role-score"><span>Fund fit</span><strong>${fit.overall==null?'—':fit.overall}</strong><small>${fit.overall==null?'dados insuficientes':fundScoreLabel(fit.overall)}</small></div>
      ...
      <section class="fund-dimension-grid">
        ${[
          ['Cost', fit.costScore, Number.isFinite(Number(r.expense_ratio)) ? fmtExpenseRatio(r.expense_ratio) : 'TER a enriquecer'],
          ['Size', fit.sizeScore, r.fund_total_assets == null ? 'AUM a enriquecer' : fmtCap(r.fund_total_assets)],
          ['Diversification', fit.diversificationScore, holdings.length ? `${holdings.length} holdings observadas` : 'holdings a enriquecer'],
          ['Concentration', fit.concentrationScore, holdings.length ? `Top 5 ${pctFundWeight(fit.top5)}` : 'holdings a enriquecer'],
          ['Overlap', fit.maxOverlap == null ? null : Math.max(0, Math.round((1-fit.maxOverlap)*100)), fit.maxOverlap == null ? 'a enriquecer' : `${overlapText} máx.`]
        ].map(([label,score,note])=>`<div><span>${label}</span><strong>${score==null?'—':score}</strong><small>${escapeHtml(note)}</small></div>`).join('')}
      </section>
    ...
    `;
    els.detail.hidden = false;
    requestAnimationFrame(()=>drawFundPriceChart(r));
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      navigator.serviceWorker.register("sw.js?v=0.98.0").then(reg => reg.update()).catch(err => console.warn("SW registration failed", err));
    });
  }
